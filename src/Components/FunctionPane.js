...
const FilterField = ({ id, label, callType }) => {
  ...
  const handleClick = () => { //HandleClick was written with help from ChatGPT, using the prompt "How do I make an HTTP GET request with a materialUI button in react?" as well as copycat.dev/blog/react-fetch
    (fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?${callType}=${value}`)
  ...
  return (
    <div>
      <Paper elevation={2}>
        ...
        <FilterField id='CocktailsByIngredientField' label='Filter Cocktails By Ingredient' callType={'i'}/>
        <p>
